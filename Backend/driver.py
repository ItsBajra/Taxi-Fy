...
from .backend import login_driver, register_driver
from .driver_dashboard import show_dashboard

# driver registration and login window
def driver_class(window, bg_label, second_frame):
    window_width = 1000
    window_height = 800
    ...
    # ---Login Function---
    def login_function():
        account_info = login_driver(email=email_login_entry.get(), password=password_login_entry.get())
        if (account_info != None):
            if (account_info[12] == 'Verified'):
                welcome_label.place_forget()
                logo_label.place_forget()
            ...
            elif (account_info[12] == 'Pending'):
                messagebox.showerror('Taxi-Fy says', 'Please wait! Your Account verification is still pending!')

            elif (account_info[12] == 'Rejected'):
                messagebox.showerror('Taxi-Fy says',
                                     'Your Account has been rejected! Please register again with correct information!')
    ...
    def registration_function():
        ...
        license_number_value = license_number_entry.get()
        vehicle_model_value = vehicle_model_entry.get()
        vehicle_registration_value = vehicle_registration_entry.get()
        password_ = password_entry.get()
        repeat_password_ = repeat_password_entry.get()

        if not all([first_name, last_name, email_, gender_, age_value, address_, phone_number, license_number_value,
                    vehicle_model_value, vehicle_registration_value, password_]):
            ...
            if age_value > 16 and age_value < 100:
                if phone_number.isdigit() and len(phone_number) == 10:
                    if password_ == repeat_password_:
                        registration = register_driver(first_name, last_name, email_, gender_, age_value, address_,
                                                       phone_number,
                                                       license_number_value, vehicle_model_value,
                                                       vehicle_registration_value,
                                                       password_)
    ...
    # license number
    license_number_frame = ctk.CTkFrame(sign_up_frame, width=300, height=55, border_width=2, border_color='gray',
                                        corner_radius=10)
    license_number_frame.grid(row=8, column=1, stick='n', pady=5)
    license_number = ctk.CTkLabel(license_number_frame, text='license Number', text_color='gray', font=('Ariel', 12),
                                  height=10)
    license_number.place(relx=0.035, rely=0.2, anchor='w')

    # license_number entry
    license_number_entry = ctk.CTkEntry(license_number_frame, border_width=0, width=290, height=30, fg_color='#2b2b2b',
                                        font=('Verdana', 14))
    license_number_entry.place(relx=0.5, rely=0.6, anchor='center')
    license_number_entry.bind('<Enter>', lambda event: on_enter_entry(event, license_number_entry))
    license_number_entry.bind('<Leave>', lambda event: on_leave_entry(event, license_number_entry))

    # vehicle model
    vehicle_model_frame = ctk.CTkFrame(sign_up_frame, width=300, height=55, border_width=2, border_color='gray',
                                       corner_radius=10)
    vehicle_model_frame.grid(row=9, column=1, stick='n', pady=5)
    vehicle_model = ctk.CTkLabel(vehicle_model_frame, text='Vehicle Model', text_color='gray', font=('Ariel', 12),
                                 height=10)
    vehicle_model.place(relx=0.035, rely=0.2, anchor='w')

    # vehicle_model entry
    vehicle_model_entry = ctk.CTkEntry(vehicle_model_frame, border_width=0, width=290, height=30, fg_color='#2b2b2b',
                                       font=('Verdana', 14))
    vehicle_model_entry.place(relx=0.5, rely=0.6, anchor='center')
    vehicle_model_entry.bind('<Enter>', lambda event: on_enter_entry(event, vehicle_model_entry))
    vehicle_model_entry.bind('<Leave>', lambda event: on_leave_entry(event, vehicle_model_entry))

    # Vehicle Registration
    vehicle_registration_frame = ctk.CTkFrame(sign_up_frame, width=300, height=55, border_width=2, border_color='gray',
                                              corner_radius=10)
    vehicle_registration_frame.grid(row=10, column=1, stick='n', pady=5)
    vehicle_registration = ctk.CTkLabel(vehicle_registration_frame, text='Vehicle Reg. Number', text_color='gray',
                                        font=('Ariel', 12),
                                        height=10)
    vehicle_registration.place(relx=0.035, rely=0.2, anchor='w')

    # vehicle_registration entry
    vehicle_registration_entry = ctk.CTkEntry(vehicle_registration_frame, border_width=0, width=290, height=30,
                                              fg_color='#2b2b2b',
                                              font=('Verdana', 14))
    vehicle_registration_entry.place(relx=0.5, rely=0.6, anchor='center')
    vehicle_registration_entry.bind('<Enter>', lambda event: on_enter_entry(event, vehicle_registration_entry))
    vehicle_registration_entry.bind('<Leave>', lambda event: on_leave_entry(event, vehicle_registration_entry))

    # password
    password_frame = ctk.CTkFrame(sign_up_frame, width=300, height=55, border_width=2, border_color='gray',
                                  corner_radius=10)
    password_frame.grid(row=11, column=1, stick='n', pady=5)
    ...
    repeat_password_frame = ctk.CTkFrame(sign_up_frame, width=300, height=55, border_width=2, border_color='gray',
                                         corner_radius=10)
    repeat_password_frame.grid(row=12, column=1, stick='n', pady=5)
    ...
    show_password.grid(row=13, column=1, stick='e', pady=5)
    ...
    submit_btn.grid(row=14, column=1, stick='w', pady=5)

    # quit button
    quit_btn = ctk.CTkButton(sign_up_frame, text='Quit', text_color='WHITE', width=140,
                             corner_radius=20, height=40, command=quit_fun)
    quit_btn.grid(row=14, column=1, stick='e', pady=5)

    # already have an account button
    registration_button = ctk.CTkButton(sign_up_frame, text='Already have an account? Click me',
                                        text_color='WHITE', width=200, corner_radius=20,
                                        height=40, command=switch_login)
    registration_button.grid(row=16, column=1, stick='s', pady=5)
    ...
    login_button = ctk.CTkButton(tabview.tab('S I G N    I N'), text='Create an account? Click Me', font=('Ariel', 13),
                                 corner_radius=20, width=200, height=40, command=switch_registration)
